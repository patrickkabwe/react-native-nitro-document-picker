...
namespace margelo::nitro::nitrodocumentpicker {
  ...
  public:
    ...
    bool equals(const std::shared_ptr<HybridObject>& other) override {
      if (auto otherCast = std::dynamic_pointer_cast<HybridNitroDocumentPickerSpecSwift>(other)) {
  ...
  public:
    // Methods
    inline std::shared_ptr<Promise<std::variant<std::vector<NitroDocumentPickerResult>, NitroDocumentPickerResult>>> pick(const NitroDocumentPickerOptions& options) override {
      auto __result = _swiftPart.pick(std::forward<decltype(options)>(options));
